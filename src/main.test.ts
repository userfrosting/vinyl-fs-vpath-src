...
test("Throws if no files resolved", async t => {
    await t.throwsAsync(
        async () => await getStream.array(new VPathSrc("./test-data/scripts-0/**/*")),
        null,
        "No files found",
    );

    await t.throwsAsync(
        async () => await getStream.array(new VPathSrc("./test-data/scripts-1/0.js")),
        null,
        "No files found",
    );
});
...
test("Pushes expected files into stream", async t => {
    const data = await getStream.array(new VPathSrc("./test-data/**/*.js"));
